...
function reducer(s:State,a:Action):State{
 ...
 case"ADD":{if(a.product.stock<=0)return s;const e=s.cart.find(i=>i.kind==="local"&&i.product.id===a.product.id&&i.sizeId===a.sizeId);if(e&&e.kind==="local"){if(e.quantity>=a.product.stock)return s;return{...s,cart:s.cart.map(i=>i.kind==="local"&&i.product.id===a.product.id&&i.sizeId===a.sizeId?{...i,quantity:i.quantity+1}:i)};}return{...s,cart:[...s.cart,{kind:"local",product:a.product,quantity:1,sizeId:a.sizeId,sizeName:a.sizeName,sizePrice:a.sizePrice}]};}
 case"SET_QTY":if(a.quantity<=0)return{...s,cart:s.cart.filter(i=>!(i.kind==="local"&&i.product.id===a.productId&&i.sizeId===a.sizeId))};return{...s,cart:s.cart.map(i=>i.kind==="local"&&i.product.id===a.productId&&i.sizeId===a.sizeId?{...i,quantity:Math.min(a.quantity,i.product.stock)}:i)};
 ...
 case"LOAD_ORDER":{const cartItems:CartItem[]=a.order.items.map(oi=>({kind:"server",itemId:oi.id,productId:oi.product_id,productName:oi.product_name,sizeName:oi.size_name,price:oi.price,lineTotal:oi.line_total,quantity:oi.quantity,batchId:oi.batch_id,sentAt:oi.sent_at}));return{...s,cart:cartItems,order:a.order};};
 }
}
